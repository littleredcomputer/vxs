...
#define INSN_OPCODE(_insn) ((_insn)->InsnValue()->opcode)
#define SET_OPCODE(_insn, value) ((_insn)->InsnValue()->opcode = (value))
#define INSN_COUNT(_insn) ((_insn)->InsnValue()->count)
#define LEXA_ESKIP(_insn) ((_insn)->InsnValue()->payload.i >> 16)
#define LEXA_BSKIP(_insn) ((_insn)->InsnValue()->payload.i & 0xffff)
...
static vm_op optab[] = {
    ...
    OPCODE("nil", OP_NONE),        OPCODE("subr", OP_SUBR),
    OPCODE("gref", OP_SYMBOL),     OPCODE("gset", OP_SYMBOL),
    OPCODE("lref", OP_LEXADDR), // 5
    OPCODE("lset", OP_LEXADDR),    OPCODE("goto", OP_INT),
    OPCODE("false?p", OP_INT),     OPCODE("false?", OP_INT),
    OPCODE("true?p", OP_INT), // 10
    OPCODE("true?", OP_INT),       OPCODE("proc", OP_NONE),
    OPCODE("extend", OP_INT),      OPCODE("extend!", OP_NONE),
    OPCODE("extend.", OP_INT), // 15
    OPCODE("save", OP_INT),        OPCODE("return", OP_NONE),
    OPCODE("pop", OP_NONE),        OPCODE("dup", OP_NONE),
    OPCODE("take", OP_INT), // 20
    OPCODE("cc", OP_NONE),         OPCODE("resume", OP_NONE),
    OPCODE("apply.", OP_NONE),     OPCODE("apply", OP_INT),
    OPCODE("unspc", OP_NONE), // 25
    OPCODE("unassn", OP_NONE),     OPCODE("lit", OP_INT),
    OPCODE("vector-set!", OP_INT), // starting here:
    OPCODE("vector-ref", OP_INT),  // scheme primitives allocated to opcode
    OPCODE("car", OP_INT),         // 30
    OPCODE("cdr", OP_INT),         OPCODE("zero?", OP_INT),
    OPCODE("+", OP_INT),           OPCODE("*", OP_INT),
    OPCODE("quotient", OP_INT), // 35
    OPCODE("remainder", OP_INT),   OPCODE("-", OP_INT),
    OPCODE("not", OP_INT),         OPCODE("null?", OP_INT),
    OPCODE("eq?", OP_INT), // 40
    OPCODE("pair?", OP_INT),       OPCODE("cons", OP_INT),
    OPCODE("gref.", OP_INT),       OPCODE("false", OP_NONE),
    OPCODE("true", OP_NONE), // 45
    OPCODE("int", OP_INT),         OPCODE("promise", OP_NONE),
    OPCODE("gset.", OP_INT),
};
...
void Context::print_insn(int addr, const Cell *insn) {
  ...
  switch (op->opnd_type) {
  case OP_INT:
    printf("%ld", insn->InsnValue()->payload.i);
    break;
  case OP_SYMBOL:
    printf("%s", insn->InsnValue()->Symbol()->key);
    break;
  case OP_SUBR:
    printf("%u,", INSN_COUNT(insn));
    printf("%s", (insn->flag(Cell::QUICK) ? insn->InsnValue()->payload.f->name
                                          : insn->InsnValue()->Symbol()->key));
    // XXX comment
    break;
  case OP_LEXADDR:
    printf("%ld,%ld", LEXA_ESKIP(insn), LEXA_BSKIP(insn));
    break;
  case OP_NONE:;
  }
  printf("\n");
...
XEQ:
  ...
  switch (opcode) {
  case 0: // consti
    save_i(insn->InsnValue()->payload.i);
    break;
  case 1: // nil
    m_stack.push(nil);
    break;
  case 2: // subr
    if (!insn->flag(Cell::QUICK)) {
      ...
      if (type == Cell::Cproc) {
        ...
        insn->InsnValue()->payload.f = cdr(subr)->SubrValue();
    ...
    save(r_envt);
    save(r_cproc);
    r_val = insn->InsnValue()->payload.f->subr(this, r_val);
    restore(r_cproc);
    restore(r_envt);
    m_stack.push(r_val);
    break;
  case 3: { // gref
    ...
    if (!r_val) {
      ...
      insn->InsnValue()->payload.i = index;
  ...
  case 12: // proc
    ...
    break;
  case 13: // extend
    if (n_args < insn->InsnValue()->payload.i)
      error("vm: not enough arguments to procedure");
    r_envt = extend_from_vector(r_envt, &m_stack, insn->InsnValue()->payload.i);
    // r_envt = extend (r_envt, gc_protect (pop_list
    // (insn->InsnValue()->payload.i))); gc_unprotect ();
    break;
  case 14: // extend!
  ...
  case 15: // extend.
    if (n_args < insn->InsnValue()->payload.i)
      error("vm: not enough arguments to procedure");
    r_val = pop_list(n_args - insn->InsnValue()->payload.i);
    r_envt = extend(r_envt, gc_protect(pop_list(insn->InsnValue()->payload.i)));
    gc_unprotect();
    adjoin(r_envt, r_val);
    break;
  case 16: // save
  ...
  case 20: { // take
    ...
    intptr_t target = insn->InsnValue()->payload.i;
  ...
  case 23: // apply.
    ...
    insn->InsnValue()->payload.i = count;
  /* FALL THROUGH */
  case 24: // apply
    r_exp = m_stack.pop();
    type = r_exp->type();
    if (type == Cell::Cproc) {
      n_args = insn->InsnValue()->payload.i;
      r_cproc = r_exp;
      goto PROC;
    } else if (type == Cell::Subr) {
      r_val = pop_list(insn->InsnValue()->payload.i);
      save(r_envt);
      save(r_cproc);
  ...
  case 26: // unassn
    ...
    break;
  case 27: // lit
    m_stack.push(literals->get(insn->InsnValue()->payload.i));
    break;
  case 28: { // vector-set!
    n_args = insn->InsnValue()->payload.i;
    if (n_args != 3)
  ...
  case 43: { // gref. (quickened global ref; contains index of target binding)
    m_stack.push(cdr(root_bindings->get(insn->InsnValue()->payload.i)));
    break;
  }
  case 44: // false
  ...
  case 45: // true
    ...
    break;
  case 46: // int
    m_stack.push(make_int(insn->InsnValue()->payload.i));
    break;
  case 47: // promise
    restore_i(start);
    ...
    break;
  case 48: // gset.
    Cell::setcdr(root_bindings->get(insn->InsnValue()->payload.i),
                 m_stack.pop());
    break;
  default:
...
Cell *Context::make_instruction(int opcode, Cell *operands) {
  ...
  case OP_LEXADDR:
    ...
    c->InsnValue()->payload.i = ((u1 << 16) | u2);
    break;
  case OP_NONE:
    break;
  default:
  ...
  for (int ix = 0; ix < insns->size(); ++ix) {
    ...
    if (opcode == 43) {                 // XXX magic number
      ...
      write_escaped_string(output,
                           car(root_bindings->get(insn->InsnValue()->payload.i))
                               ->SymbolValue()
                               ->key);
    } else if (opcode == 48) {
      ...
      case OP_NONE:
        ...
        break;
      case OP_INT:
        fprintf(output, "0,(void*)%" PRIdPTR, insn->InsnValue()->payload.i);
        break;
      case OP_SYMBOL:
        fprintf(output, "0,");
        write_escaped_string(output, insn->InsnValue()->Symbol()->key);
        break;
      case OP_SUBR:
        // XXX write a comment
        fprintf(output, "%u, ", INSN_COUNT(insn));
        if (insn->flag(Cell::QUICK))
          write_escaped_string(output, insn->InsnValue()->payload.f->name);
        else
          write_escaped_string(output, insn->InsnValue()->Symbol()->key);
        break;
      case OP_LEXADDR:
        fprintf(output, "0,(void*)%#" PRIxPTR, insn->InsnValue()->payload.i);
        break;
      }
    }
    fprintf(output, " },\n");
