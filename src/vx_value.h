...
struct Value {
  ...
  static inline Value from_double(double d) {
    ...
    // If it's a NaN, canonicalize to a positive quiet NaN so it doesn't collide with tags
    if ((bits & 0x7FF8000000000000ULL) == 0x7FF0000000000000ULL && (bits & 0x0007FFFFFFFFFFFFULL) != 0) {
  ...
  // Pointer
  template <typename T>
  static inline Value from_ptr(const T *ptr) {
  ...
  inline bool is_bool() const {
    return raw == TAG_FALSE || raw == TAG_TRUE;
  }
  ...
  inline int32_t as_int() const {
    if (is_int()) return static_cast<int32_t>(raw & INT_MASK);
    if (is_double()) return static_cast<int32_t>(as_double());
    return 0;
  }
...
static_assert(std::is_trivially_copyable_v<Value>, "Value must be trivially copyable for register/memcpy speed");
